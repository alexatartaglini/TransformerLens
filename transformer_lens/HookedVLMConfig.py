...
:class:`transformer_lens.HookedVLM` model.
"""
...
@dataclass
class HookedVLMConfig:
    """
    Configuration class to store the configuration of a HookedVLM model.

    See further_comments.md for more details on the more complex arguments.

    Args:
        d_model_language (int): The dimensionality of the embeddings for the language model.
        d_head_language (int): The dimensionality of each attention head for the language model.
        n_layers_language (int): The number of transformer blocks (one block = one attn layer AND one MLP layer) for the language model.
        n_ctx_language (int): The maximum sequence length for the language model.
        n_heads_language (int): The number of attention heads for the language model. If not
            specified, will be set to d_model_language // d_head_language. (This is represented by a default value of -1)
        d_mlp_language (int, *optional*): The dimensionality of the feedforward mlp
            network for the language model. Defaults to 4 * d_model_language, and in an attn-only model is None.
        ...
        act_fn_language (str, *optional*): The activation function to use for the language model. Always
            lowercase. Supports ['relu', 'gelu', 'silu', 'gelu_new', 'solu_ln',
            'gelu_fast']. Must be set unless using an attn-only model.
        eps_language (float): The epsilon value to use for layer normalization for the language model. Defaults
            to 1e-5
        ...
        processor_name (str, *optional*): the full name of the model, passed into
            HuggingFace to access the processor. Only used when passing in
        ...
        normalization_type_language (str, *optional*): the type of normalization to use for the language model.
        ...
        positional_embedding_type (str): The positional embedding used. Options
            ...
            currently supported. Defaults to 'rotary'.
        ...
        NTK_by_parts_factor (float): The overall factor used in the "NTK-by-parts" method that
            ...
            Defaults to 8.0.
        d_model_vision (int): The dimensionality of the embeddings for the vision model.
        d_head_vision (int): The dimensionality of each attention head for the vision model.
        n_layers_vision (int): The number of transformer blocks (one block = one attn layer AND one MLP layer) for the vision model.
        n_layers_global (int): The number of transformer blocks (one block = one attn layer AND one MLP layer) for the global model.
        n_ctx_vision (int): The maximum sequence length for the vision model.
        n_heads_vision (int): The number of attention heads for the vision model. If not
            specified, will be set to d_model_vision // d_head_vision. (This is represented by a default value of -1)
        d_mlp_vision (int, *optional*): The dimensionality of the feedforward mlp
            network for the vision model. Defaults to 4 * d_model_vision, and in an attn-only model is None.
        act_fn_vision (str, *optional*): The activation function to use for the vision model. Always
            lowercase. Supports ['relu', 'gelu', 'silu', 'gelu_new', 'solu_ln',
            'gelu_fast']. Must be set unless using an attn-only model.
        normalization_type_vision (str, *optional*): the type of normalization to use for the vision model.
            Options are None (no normalization), 'LN' (use LayerNorm, including weights
            & biases) and 'LNPre' (use LayerNorm, but no weights or biases), 'RMS'
            (use RMSNorm, including weights) and 'RMSPre' (use RMSNorm, but no weights or biases).
            Defaults to LN
        eps_vision (float): The epsilon value to use for layer normalization for the vision model. Defaults
            to 1e-5
        image_size (int): The size of the image.
        patch_size (int): The size of the patch.
        num_channels (int): The number of channels in the image.
        output_dim_vision (int): The dimensionality of the output for the vision model.
        cross_attn_layers (List[int]): The layers at which cross-attention is used.
        vision_intermediate_layer_idx (List[int]): The layers at which the vision model is added to the residual stream.
    """

    n_layers_language: int
    d_model_language: int
    n_ctx_language: int
    d_head_language: int
    d_model_vision: int
    d_head_vision: int
    ...
    n_heads_language: int = -1
    d_mlp_language: Optional[int] = None
    act_fn_language: Optional[str] = None
    normalization_type_language: Optional[str] = "RMS"
    d_vocab: int = -1
    eps_language: float = 1e-5
    ...
    final_rms_language: bool = True
    final_rms_vision: bool = False
    pre_global_layer_norm: bool = True
    d_vocab_out: int = -1
    ...
    d_mlp_vision: Optional[int] = None
    act_fn_vision: Optional[str] = None
    normalization_type_vision: Optional[str] = "LN"
    eps_vision: float = 1e-5
    output_dim_vision: Optional[int] = None
    cross_attn_layers: Optional[List[int]] = None
    vision_intermediate_layer_idx: Optional[List[int]] = None
    act_fn: Optional[str] = None
    modality: Optional[str] = None

    def __post_init__(self):
        if self.n_heads_language == -1:
            self.n_heads_language = self.d_model_language // self.d_head_language

        if not self.d_model_language % (self.d_head_language) == 0:
            logging.warning(
                "d_model_language %d is not divisible by d_head_language %d."
                "n_heads_language was inferred to be %d, rounding down the ratio.",
                self.d_model_language,
                self.d_head_language,
                self.n_heads_language,
                )
        ...
        if not self.attn_only:
            if self.d_mlp_language is None:
                # For some reason everyone hard codes in this hyper-parameter!
                self.d_mlp_language: int = self.d_model_language * 4
            assert self.act_fn_language is not None, "act_fn_language must be specified for non-attn-only models"
            assert (
                self.act_fn_language in SUPPORTED_ACTIVATIONS
            ), f"act_fn_language={self.act_fn_language} must be one of {SUPPORTED_ACTIVATIONS}"
        if self.initializer_range < 0 and self.init_mode == "gpt2":
            # Roughly copy the GPT-2 value, but proportional to sqrt(1/d_model)
            self.initializer_range = 0.8 / np.sqrt(self.d_model_language)
        if self.initializer_range < 0 and self.init_mode != "gpt2":
        ...
        if self.d_vocab_out == -1:
            ...
            # If d_vocab is not set, it'll be inferred from processor_name or from a processor
        ...
        if self.positional_embedding_type == "rotary" and self.rotary_dim is None:
            self.rotary_dim = self.d_head_language
        ...
        self.n_params_language = self.n_layers_language * ((self.d_model_language * self.d_head_language * self.n_heads_language * 4))
        if not self.attn_only:
            assert self.d_mlp_language is not None  # mypy
            # Number of parameters in MLP layers (ignoring biases and layer norm). 2 because W_in and W_out
            mlp_params_per_layer = self.d_model_language * self.d_mlp_language * (2 + self.gated_mlp)

            if self.num_experts:
                # If we are using MoE, we multiply by num_experts, and add the expert gate parameters (d_model * num_experts)
                mlp_params_per_layer = (mlp_params_per_layer + self.d_model_language) * self.num_experts
            self.n_params_language += self.n_layers_language * mlp_params_per_layer

        # TODO: vision params, fix below
        ...
        if self.use_attn_scale and self.attn_scale == -1.0:
            self.attn_scale = np.sqrt(self.d_head_language)
    ...
    @classmethod
    def unwrap(cls, config: Union[Dict, "HookedVLMConfig"]) -> HookedVLMConfig:
        """
        Convenience function to avoid duplicate code from a common way config is passed to various components
        """
        return HookedVLMConfig.from_dict(config) if isinstance(config, Dict) else config

    @classmethod
    def from_dict(cls, config_dict: Dict[str, Any]) -> HookedVLMConfig:
        """
        Instantiates a `HookedVLMConfig` from a Python dictionary of
        parameters.
        """
