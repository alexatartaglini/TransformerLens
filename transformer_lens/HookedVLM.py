...
Closely follows the implementation of :class:`transformer_lens.HookedTransformer` with a few 
VLM-specific alterations.
"""
...
from transformers import AutoModelForCausalLM, AutoProcessor, MllamaProcessor
...
# generation.
from transformer_lens.past_key_value_caching import HookedTransformerKeyValueCache  # TODO: make VLM-specific
...
T = TypeVar("T", bound="HookedVLM")


class Output(NamedTuple):  # TODO: make VLM-specific
    """Output Named Tuple.
...
class HookedVisionModel(HookedRootModule):
    def __init__(self, cfg: HookedVLMConfig):
        super().__init__()
        self.cfg = self.convert_to_vision_cfg(cfg)

        """
        self.patch_embed = PatchEmbed(self.cfg)
        self.hook_patch_embed = HookPoint()  # [batch, pos, d_model]

        self.cls_token = nn.Parameter(torch.zeros(1, 1, self.cfg.d_model_vision, dtype=self.cfg.dtype))
        self.hook_cls_token = HookPoint()  # [batch, 1, d_model]

        self.gated_pos_embed = GatedPosEmbed(self.cfg)
        self.hook_gated_pos_embed = HookPoint()  # [batch, pos, d_model]
        """


        # TODO: patch embedding, cls token, gated pos embed, tile embed, pre tile pos embed, post tile pos embed, ln_pre, ln_post
        

    def convert_to_vision_cfg(self, cfg: HookedVLMConfig) -> HookedVLMConfig:
        """Convert a HookedVLMConfig to a vision model HookedVLMConfig."""
        cfg = copy.deepcopy(cfg)
        cfg.d_model = cfg.d_model_vision
        cfg.d_head = cfg.d_head_vision
        cfg.n_layers = cfg.n_layers_vision
        cfg.n_ctx = cfg.n_ctx_vision
        cfg.n_heads = cfg.n_heads_vision
        cfg.d_mlp = cfg.d_mlp_vision
        cfg.act_fn = cfg.act_fn_vision
        cfg.eps = cfg.eps_vision
        cfg.normalization_type = cfg.normalization_type_vision
        cfg.modality = "vision"
        cfg.final_rms = cfg.final_rms_vision
        return cfg
...
class HookedLanguageModel(HookedRootModule):
    def __init__(self, cfg: HookedVLMConfig):
        super().__init__()
        self.cfg = self.convert_to_language_cfg(cfg)

        self.embed = Embed(self.cfg, additional_tokens=8)
        ...
        blocks = [
            TransformerBlock(self.cfg, block_index) for block_index in range(self.cfg.n_layers_language) if block_index not in self.cfg.cross_attn_layers
        ]

        if self.cfg.cross_attn_layers is not None:
            for layer_index in self.cfg.cross_attn_layers:
                blocks.insert(layer_index, CrossAttentionBlock(self.cfg, layer_index))

        self.blocks = nn.ModuleList(blocks)

        if self.cfg.normalization_type_language == "RMS":
            self.ln_final = RMSNorm(self.cfg)
        elif self.cfg.normalization_type_language == "RMSPre":
            self.ln_final = RMSNormPre(self.cfg)
        elif self.cfg.normalization_type_language == "LN":
            if self.cfg.final_rms:
                self.ln_final = RMSNorm(self.cfg)
            else:
                self.ln_final = LayerNorm(self.cfg)
        elif self.cfg.normalization_type_language == "LNPre":
            # We've folded in LayerNorm weights, so just need the center + scale parts
            if self.cfg.final_rms:
                self.ln_final = RMSNormPre(self.cfg)
            else:
                self.ln_final = LayerNormPre(self.cfg)
        elif self.cfg.normalization_type_language is None:
            # If it's None, don't create either layer
            pass
        else:
            logging.warning("Invalid normalization_type_language passed in %s", self.cfg.normalization_type_language)
        self.unembed = Unembed(self.cfg)

    def convert_to_language_cfg(self, cfg: HookedVLMConfig) -> HookedVLMConfig:
        """Convert a HookedVLMConfig to a language model HookedVLMConfig."""
        cfg = copy.deepcopy(cfg)
        cfg.d_model = cfg.d_model_language
        cfg.d_head = cfg.d_head_language
        cfg.n_layers = cfg.n_layers_language
        cfg.n_ctx = cfg.n_ctx_language
        cfg.n_heads = cfg.n_heads_language
        cfg.d_mlp = cfg.d_mlp_language
        cfg.act_fn = cfg.act_fn_language
        cfg.eps = cfg.eps_language
        cfg.normalization_type = cfg.normalization_type_language
        cfg.modality = "language"
        cfg.final_rms = cfg.final_rms_language
        return cfg
...
class HookedVLM(HookedRootModule):
    def __init__(
        self,
        cfg: HookedVLMConfig,
        ...
        if processor is not None:
            self.set_processor(processor, default_padding_side=default_padding_side)
        elif self.cfg.processor_name is not None:
            # If we have a tokenizer name, we can load it from HuggingFace
            if self.cfg.processor_name in NON_HF_HOSTED_MODEL_NAMES:
                logging.warning(
                    "%s processor not loaded. Please load manually.",
                    self.cfg.processor_name,
                )
            else:
            ...
            # If no processor name is provided, we assume we're training on an algorithmic task and
            # will pass in tokens directly. In this case, we don't need a processor.
            assert self.cfg.d_vocab != -1, "Must provide a processor if d_vocab is not provided"
            self.processor = None
            if default_padding_side != "right":
                logging.warning(
                    "default_padding_side is explictly given but ignored because processor is not set."
                )

        self.language_model = HookedLanguageModel(self.cfg)
        self.vision_model = HookedVisionModel(self.cfg)
    ...
    def input_to_embed(
        ...
        inputs: Optional[BatchFeature] = None,
        ...
        Args:
            text (Optional[Union[str, List[str]]]): The text input to the model.
            image (Optional[Union[PIL.Image.Image, List[PIL.Image.Image]]]): The image input to the model.
            inputs (Optional[BatchFeature]): The processed inputs from the processor.
            template (bool): Whether to use the instruction template for the input.
        ...
        """
        assert not (inputs is not None and (text is not None or image is not None)), "Cannot pass both text and image and processed inputs"
        assert not (inputs is None and (text is None or image is None)), "Must pass either text and image or processed inputs"

        if inputs is not None:
            assert self.processor is not None, "Must provide a processor if passing raw inputs to the model"

            if isinstance(text, str) and isinstance(image, list):
                text = [text for _ in range(len(image))]
            elif isinstance(text, list) and isinstance(image, str):
                image = [image for _ in range(len(text))]
            elif isinstance(text, list) and isinstance(image, list):
                assert len(text) == len(image), "text and image must have the same length"
            
            inputs = self.to_tokens_and_pixel_values(text, image, template=template)
        
        if inputs.device.type != self.cfg.device:
            inputs = inputs.to(devices.get_device_for_block_index(0, self.cfg))

        if self.cfg.use_hook_tokens:
            inputs.input_ids = self.hook_tokens(inputs.input_ids)
    ...
    def set_processor(
        ...
        """Set the processor to use for this model.

        Args:
            processor (AutoProcessor): a pretrained HuggingFace processor.
            default_padding_side (str): "right" or "left", which side to pad on.

        """
        assert isinstance(
            processor, (AutoProcessor, MllamaProcessor)
        ), f"{type(processor)} is not a supported processor, please use AutoProcessor or MllamaProcessor"
        ...
        assert self.processor is not None  # keep mypy happy
    ...
    def to_tokens_and_pixel_values(
        self,
        ...
        Args:
            text (Union[str, List[str]]): The text input to the model.
            image (Union[PIL.Image.Image, List[PIL.Image.Image]]): The image input to the model.
            template (bool): Whether to use the instruction template for the input.
        ...
        """
        assert self.processor is not None, "Cannot use to_tokens without a processor"
    ...
    def move_model_modules_to_device(self):
        self.language_model.move_model_modules_to_device()
        self.vision_model.move_model_modules_to_device()
    ...
    def from_pretrained(
        ...
        **from_pretrained_kwargs,
        ...
        Load in pretrained model weights to the HookedVLM format and optionally to do some
        processing to make the model easier to interpret. Currently supports loading from 
        HuggingFace LLaMA 3.2-Vision models.
        ...
        >>> from transformer_lens import HookedVLM
        >>> model = HookedVLM.from_pretrained("meta-llama/Llama-3.2-11B-Vision-Instruct")
        Loaded pretrained model meta-llama/Llama-3.2-11B-Vision-Instruct into HookedVLM
        ...
        Args:
            ...
            processor: The processor to use for the model. If not
                provided, it is inferred from cfg.processor_name or initialized to None. If None,
    ...
    def _init_weights_gpt2(self):
        ...
        all_params = [
            ...
            self.language_model.named_parameters(),
    ...
    def load_and_process_state_dict(
        ...
        if fold_ln:
            pass 
            # state_dict = self.fold_layer_norm(state_dict)  # TODO: fix this

        if center_writing_weights:
            pass
            #state_dict = self.center_writing_weights(state_dict)  # TODO: fix this
    ...
    def fold_layer_norm(
        ...
        submodules = ["language_model", "vision_model.transformer", "vision_model.global"]

        for submodule in submodules:
            if submodule == "language_model":
                n_layers = self.cfg.n_layers_language
                act_fn = self.cfg.act_fn_language
                fold_biases = False
                center_weights = False
            elif submodule == "vision_model.transformer":
                n_layers = self.cfg.n_layers_vision
                act_fn = self.cfg.act_fn_vision
                fold_biases = True
                center_weights = True
            else:
                n_layers = self.cfg.n_layers_global
                act_fn = self.cfg.act_fn_vision
                fold_biases = True
                center_weights = True

            for l in range(n_layers):
                if submodule == "language_model" and l in self.cfg.cross_attn_layers:
                    attn = "cross_attn"
                else:
                    attn = "attn"
                ...
                if fold_biases:
                    state_dict[f"{submodule}.blocks.{l}.{attn}.b_Q"] = state_dict[f"{submodule}.blocks.{l}.{attn}.b_Q"] + (
                        state_dict[f"{submodule}.blocks.{l}.{attn}.W_Q"]
                        * state_dict[f"{submodule}.blocks.{l}.ln1.b"][None, :, None]
                    ).sum(-2)
                    state_dict[f"{submodule}.blocks.{l}.{attn}.{gqa}b_K"] = state_dict[
                        f"{submodule}.blocks.{l}.{attn}.{gqa}b_K"
                    ] + (
                        state_dict[f"{submodule}.blocks.{l}.{attn}.{gqa}W_K"]
                        * state_dict[f"{submodule}.blocks.{l}.ln1.b"][None, :, None]
                    ).sum(
                        -2
                    )
                    state_dict[f"{submodule}.blocks.{l}.{attn}.{gqa}b_V"] = state_dict[
                        f"{submodule}.blocks.{l}.{attn}.{gqa}b_V"
                    ] + (
                        state_dict[f"{submodule}.blocks.{l}.{attn}.{gqa}W_V"]
                        * state_dict[f"{submodule}.blocks.{l}.ln1.b"][None, :, None]
                    ).sum(
                        -2
                    )
                    del state_dict[f"{submodule}.blocks.{l}.ln1.b"]

                state_dict[f"{submodule}.blocks.{l}.{attn}.W_Q"] = (
                    state_dict[f"{submodule}.blocks.{l}.{attn}.W_Q"] * state_dict[f"{submodule}.blocks.{l}.ln1.w"][None, :, None]
                )

                state_dict[f"{submodule}.blocks.{l}.{attn}.{gqa}W_K"] = (
                    state_dict[f"{submodule}.blocks.{l}.{attn}.{gqa}W_K"]
                    * state_dict[f"{submodule}.blocks.{l}.ln1.w"][None, :, None]
                )
                state_dict[f"{submodule}.blocks.{l}.{attn}.{gqa}W_V"] = (
                    state_dict[f"{submodule}.blocks.{l}.{attn}.{gqa}W_V"]
                    * state_dict[f"{submodule}.blocks.{l}.ln1.w"][None, :, None]
                )
                del state_dict[f"{submodule}.blocks.{l}.ln1.w"]
                ...
                if center_weights:
                    state_dict[f"{submodule}.blocks.{l}.{attn}.W_Q"] -= einops.reduce(
                        state_dict[f"{submodule}.blocks.{l}.{attn}.W_Q"],
                        "head_index d_model d_head -> head_index 1 d_head",
                        "mean",
                    )
                    state_dict[f"{submodule}.blocks.{l}.{attn}.{gqa}W_K"] -= einops.reduce(
                        state_dict[f"{submodule}.blocks.{l}.{attn}.{gqa}W_K"],
                        "head_index d_model d_head -> head_index 1 d_head",
                        "mean",
                    )
                    state_dict[f"{submodule}.blocks.{l}.{attn}.{gqa}W_V"] -= einops.reduce(
                        state_dict[f"{submodule}.blocks.{l}.{attn}.{gqa}W_V"],
                        "head_index d_model d_head -> head_index 1 d_head",
                        "mean",
                    )
                ...
                if not self.cfg.attn_only:
                    if fold_biases:
                        state_dict[f"{submodule}.blocks.{l}.mlp.b_in"] = state_dict[f"{submodule}.blocks.{l}.mlp.b_in"] + (
                            state_dict[f"{submodule}.blocks.{l}.mlp.W_in"]
                            * state_dict[f"{submodule}.blocks.{l}.ln2.b"][:, None]
                        ).sum(-2)
                        del state_dict[f"{submodule}.blocks.{l}.ln2.b"]

                    state_dict[f"{submodule}.blocks.{l}.mlp.W_in"] = (
                        state_dict[f"{submodule}.blocks.{l}.mlp.W_in"] * state_dict[f"{submodule}.blocks.{l}.ln2.w"][:, None]
                    )

                    if self.cfg.gated_mlp and submodule == "language_model":
                        state_dict[f"{submodule}.blocks.{l}.mlp.W_gate"] = (
                            state_dict[f"{submodule}.blocks.{l}.mlp.W_gate"]
                            * state_dict[f"{submodule}.blocks.{l}.ln2.w"][:, None]
                        )

                    del state_dict[f"{submodule}.blocks.{l}.ln2.w"]

                    if center_weights:
                        # Center the weights that read in from the LayerNormPre
                        state_dict[f"{submodule}.blocks.{l}.mlp.W_in"] -= einops.reduce(
                            state_dict[f"{submodule}.blocks.{l}.mlp.W_in"],
                            "d_model d_mlp -> 1 d_mlp",
                            "mean",
                        )

                    if act_fn is not None and act_fn.startswith("solu"):
                        # Fold ln3 into activation
                        if fold_biases:
                            state_dict[f"{submodule}.blocks.{l}.mlp.b_out"] = state_dict[
                                f"{submodule}.blocks.{l}.mlp.b_out"
                            ] + (
                                state_dict[f"{submodule}.blocks.{l}.mlp.W_out"]
                                * state_dict[f"{submodule}.blocks.{l}.mlp.ln.b"][:, None]
                            ).sum(
                                -2
                            )

                            del state_dict[f"{submodule}.blocks.{l}.mlp.ln.b"]

                        state_dict[f"{submodule}.blocks.{l}.mlp.W_out"] = (
                            state_dict[f"{submodule}.blocks.{l}.mlp.W_out"]
                            * state_dict[f"{submodule}.blocks.{l}.mlp.ln.w"][:, None]
                        )

                        if center_weights:
                            # Center the weights that read in from the LayerNormPre
                            state_dict[f"{submodule}.blocks.{l}.mlp.W_out"] -= einops.reduce(
                                state_dict[f"{submodule}.blocks.{l}.mlp.W_out"],
                                "d_mlp d_model -> 1 d_model",
                                "mean",
                            )

                        del state_dict[f"{submodule}.blocks.{l}.mlp.ln.w"]

        # Fold ln_final into Unembed
        state_dict["language_model.unembed.W_U"] = state_dict["language_model.unembed.W_U"] * state_dict["language_model.ln_final.w"][:, None]
        del state_dict["language_model.ln_final.w"]
    ...
    def center_writing_weights(self, state_dict: Dict[str, torch.Tensor]):
        ...
        for submodule in submodules:
            if submodule == "language_model":
                normalization_type = self.cfg.normalization_type_language
                final_rms = self.cfg.final_rms_language
                n_layers = self.cfg.n_layers_language
            else:
                normalization_type = self.cfg.normalization_type_vision
                final_rms = self.cfg.final_rms_vision
                if submodule == "vision_model.transformer":
                    n_layers = self.cfg.n_layers_vision
                else:
                    n_layers = self.cfg.n_layers_global
